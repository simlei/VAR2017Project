...
// once the gui has been assembled, register callbacks to listen for component specific events //
    ...
    gui->onToggleEvent(this, &DatGui::onToggleEvent);
    gui->onSliderEvent(this, &DatGui::onSliderEvent);
    gui->onTextInputEvent(this, &DatGui::onTextInputEvent);
    gui->on2dPadEvent(this, &DatGui::on2dPadEvent);
    gui->onDropdownEvent(this, &DatGui::onDropdownEvent);
    gui->onColorPickerEvent(this, &DatGui::onColorPickerEvent);
    gui->onMatrixEvent(this, &DatGui::onMatrixEvent);
...
void DatGui::onTextInputEvent(ofxDatGuiTextInputEvent e)
{
    cout << "onTextInputEvent: " << e.target->getLabel() << " " << e.target->getText() << endl;
}

void DatGui::on2dPadEvent(ofxDatGui2dPadEvent e)
{
...
void DatGui::onColorPickerEvent(ofxDatGuiColorPickerEvent e)
{
    cout << "onColorPickerEvent: " << e.target->getLabel() << " " << e.target->getColor() << endl;
    ofSetBackgroundColor(e.color);
}

void DatGui::onMatrixEvent(ofxDatGuiMatrixEvent e)
{
...
void DatGui::keyPressed(int key)
{
    if (key == 'f') {
        toggleFullscreen();
    }   else if (key == 32){
